...
const PostDetail = ({ selectedPost, onClose }) => {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/comment/create/${selectedPost.postId}`,
  ...
  const handleCommentLikeClick = async (commentId, isLiked) => {
    try {
      const url = isLiked
        ? `http://localhost:8080/comment/removelike/${commentId}` // 좋아요 해제 API
        : `http://localhost:8080/comment/addcommentlike/${commentId}`; // 좋아요 API

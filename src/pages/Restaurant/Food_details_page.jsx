...
const FoodDetailPage = () => {
  ...
  const fetchStoreData = async () => {
    try {
      const response = await axios.get(
        `http://15.164.59.210:8080/restaurant/${storeId}`,
        {
          headers: accessToken
      ...
      const searchResponse = await axios.get(
        `http://15.164.59.210:8080/restaurant/search/list?search=${data.restaurantName}`
      );
      if (
        searchResponse.data &&
      ...
      if (data.isLogin && accessToken) {
        const profileResponse = await axios.get(
          `http://15.164.59.210:8080/profiles`,
          {
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        await axios.delete(
          `http://15.164.59.210:8080/restaurant/removeheart/${storeId}`,
          {
            headers: { Authorization: `Bearer ${accessToken}` }, // 헤더에 토큰 포함
          }
        );
        ...
        await axios.post(
          `http://15.164.59.210:8080/restaurant/addheart/${storeId}`,
          {},
          {
            headers: { Authorization: `Bearer ${accessToken}` }, // 헤더에 토큰 포함
          }
  ...
  const handleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        await axios.delete(
          `http://15.164.59.210:8080/restaurant/removebookmark/${storeId}`,
          {
            headers: { Authorization: `Bearer ${accessToken}` }, // 헤더에 토큰 포함
          }
        );
        ...
        await axios.post(
          `http://15.164.59.210:8080/restaurant/addbookmark/${storeId}`,
          {},
          {
            headers: { Authorization: `Bearer ${accessToken}` }, // 헤더에 토큰 포함
          }
  ...
  const handleReviewSubmit = async (updatedReviewData) => {
    try {
      await axios.post(
        `http://15.164.59.210:8080/restaurant/review/${editingReview.reviewId}`,
        updatedReviewData,
        { headers: { Authorization: `Bearer ${accessToken}` } } // 헤더에 토큰 포함
      );
  ...
  const handleReviewDelete = async (reviewId) => {
    ...
    try {
      await axios.delete(
        `http://15.164.59.210:8080/restaurant/review/${reviewId}`,
        {
          headers: { Authorization: `Bearer ${accessToken}` }, // 헤더에 토큰 포함
        }
      );
